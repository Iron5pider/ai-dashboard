...
export function usePathAnalytics(path: LearningPath): PathAnalytics {
  ...
  useEffect(() => {
    const calculateAnalytics = () => {
      ...
      const completionRate = (path.progress.completed / path.progress.totalVideos) * 100
      ...
      const timeSpentPerVideo = (path.progress.timeSpent || 0) / path.progress.completed || 0
...
function calculateStreakDays(path: LearningPath): number {
...
function calculateTopicsProgress(path: LearningPath): Map<string, number> {
  ...
  path.topics.forEach(topic => {
    ...
    const progress = topicVideos.length > 0 ? (completedVideos / topicVideos.length) * 100 : 0
...
function getNextMilestone(path: LearningPath) {
  if (!path.progress) {
    return {
      type: 'completion',
      value: 25,
  ...
  const completion = (path.progress.completed / path.progress.totalVideos) * 100
